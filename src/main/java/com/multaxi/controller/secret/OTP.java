...
class OTP {
	...
	/**
	 * Checks whether a code is valid for a specific counter with a delay window of
	 * 0
	 *
	...
	/**
	 * Checks whether a code is valid for a specific counter taking a delay window
	 * into account
	 *
	 ...
	 */
	protected URI getURI(final String type, final String issuer, final String account, final Map<String, String> query)
	 ...
	 */
	private byte[] generateHash(final byte[] secret, final byte[] data)
	...
	private String getCodeFromHash(final byte[] hash) {
		/*
		 * Find mask to get last 4 digits: 1. Set all bits to 1: ~0 -> 11111111 -> 255
		 * decimal -> 0xFF 2. Shift n (in this case 4, because we want the last 4 bits)
		 * bits to left with << 3. Negate the result: 1111 1100 -> 0000 0011
		 */
		int mask = ~(~0 << 4);

		/*
		 * Get last 4 bits of hash as offset: Use the bitwise AND (&) operator to select
		 * last 4 bits Mask should be 00001111 = 15 = 0xF Last byte of hash & 0xF = last
		 * 4 bits: Example: Input: decimal 219 as binary: 11011011 & Mask: decimal 15 as
		 * binary: 00001111 ----------------------------------------- Output: decimal 11
		 * as binary: 00001011
		 */
